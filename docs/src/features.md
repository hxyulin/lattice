...
coincidence, the exact shape an NNUE accumulator takes - the default NNUE
evaluation (below) maintains its two perspective accumulators through these very
same `put_piece` / `remove_piece` hooks.

## Evaluation

Lattice ships **two** evaluations. The default is an **NNUE** network (last
subsection below); building with `--no-default-features` selects the
**hand-crafted evaluation (HCE)** described first. The HCE's material/PST
accumulator is not just a fallback - it is the structural model the NNUE
accumulator follows, which is why it is worth understanding first.
...
Material and PST are the HCE's backbone; a mobility term (next) is the only
positional term layered on top. King safety, pawn structure, and the rest are
deliberately absent - the HCE is kept compact because the default evaluation is
the NNUE network, and the remaining HCE terms are low on the priority list (see
[Planned](#planned)).
...
The one positional term the HCE adds beyond PST: each knight, bishop, rook, and
queen scores a small bonus per square it can reach inside its **mobility area** -
squares neither occupied by a friendly piece nor attacked by an enemy pawn (a
square a piece nominally reaches but a pawn guards is not real mobility). The
per-square weights are deliberately small (2 centipawns for minors, 1-2 for
rooks and queens, tapered), a gentle ordering nudge rather than a material-scale
term; the king and pawns are not mobility-scored. The score is a White-relative
`(mg, eg)` differential blended by the same phase as the PST terms, so a
symmetric position (the start) nets to zero.

### NNUE

The **default** evaluation is a small **NNUE** (Efficiently Updatable Neural
Network): a `(768 -> 256) x2 -> 1` perspective network trained offline with
[bullet] and embedded as a quantised `i16` network at compile time. The 768
inputs are one per `(color, piece type, square)`; the network keeps **two**
accumulators, one from each side's perspective, and the forward pass reads the
side-to-move's as `us` and the other as `them` before the output layer maps the
result to centipawns.

Like the tapered accumulator, the two perspective accumulators are maintained
**incrementally** through the board's `put_piece` / `remove_piece` hooks - a
placement adds one feature's `256`-wide weight column, a removal subtracts it -
and are seeded from the mailbox on setup. Feature indexing mirrors bullet's
`Chess768` exactly so the embedded network reads correctly, and the incremental
path is tested against a from-scratch reseed as its oracle.

This lives in the board layer (beside PeSTO) for the same reason the tapered
accumulator does: the network is a pure function of piece placement, and only
the board observes every placement. The engine's `evaluate` simply calls
`Board::nnue_eval` and reads the final centipawn score.
...
Iterative deepening gives the next depth a strong prior: the previous
iteration's score. So rather than searching each root depth with a full
`(-inf, +inf)` window, the engine searches a **narrow window** centered on that
prior, `(prev - delta, prev + delta)`. A narrow window prunes far harder. If the
true score lands inside, the search is much cheaper; if the move **fails low or
high** (the score escapes the window), that side of the window is widened and the
root re-searched. Shallow iterations (below a minimum depth) and mate-range
scores, where the prior is unreliable, fall back to a full window.
...
Pruning trims the tree; **extensions** grow it back where the position demands
it. A node in check is highly forcing - few legal replies, and often a tactic or
mate hiding just past the horizon - so it is searched **one ply deeper**. The
extra depth is free to detect here (the `in_check` flag is already computed for
the pruning gates), and because RFP, NMP, LMP, and LMR are all gated on
`!in_check`, a checking node is only ever deepened, never pruned. A hard ply cap
keeps a long series of checks from overrunning the fixed-size search stacks.
...
- **Search:** SEE in move ordering, countermove and continuation history,
  classic futility pruning, razoring.
- **Evaluation (HCE):** king safety, pawn structure (passed / isolated /
  doubled), bishop pair, rook on open file - the terms the hand-crafted eval
  still lacks. Lower priority now that NNUE is the default.
- **NNUE:** a larger or retrained network, and output buckets by game phase.
