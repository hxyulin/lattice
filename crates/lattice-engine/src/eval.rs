...
//!
//! The tables are laid out a8-first (rank 8 row first, file a to h), the
...
//! table at `sq ^ 56` and Black, mirrored, reads it at `sq`.
...
/// Piece types in discriminant order, indexed in lockstep with the value,
/// phase, and table arrays.
...
pub fn evaluate(board: &Board) -> Score {
    let mut mg = 0; // from White's perspective
    let mut eg = 0;
    let mut phase = 0;
    ...
    for pt in PIECES {
        ...
        for sq in board.pieces(Color::White, pt) {
            let s = (sq.index() ^ 56) as usize;
            mg += MG_VALUE[i] + PST_MG[i][s];
            eg += EG_VALUE[i] + PST_EG[i][s];
            phase += PHASE_INC[i];
        }
    ...
    // Promotions can push phase past the starting maximum; clamp before blending.
